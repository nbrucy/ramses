module galbox_module
    use amr_parameters
    !================================================================
    ! This module contains the variable needed for galbox module
    !================================================================
    ...
    logical, save::no_inflow_boundary = .false. ! Prevent inflow in boundary condition
  ...
  subroutine read_galbox_params()
    ...
    !--------------------------------------------------
    namelist /galbox_params/ turb, Height0, dens0, Bx, By, Bz, temperature, file_init_turb, no_inflow_boundary
  ...
  subroutine galbox_condinit(x, q, dx, nn)
  
    !================================================================
    ! This routine generates initial for stratified boxes.
    !================================================================
...
!   #endif
    ...
    integer, save:: n_size ! number of cell in one direction of the turbulence initial field
    integer:: ind_i, ind_j, ind_k
    real(dp), save:: ind, seed1, seed2, seed3, xi, yi, zi
    real(dp):: n_total ! total number of cells in the turbulence initial field
  
    ! start initialising q to zero
    q = 0.
...
!   #endif
  ...
  end subroutine galbox_condinit
  
  !#########################################################
  !#########################################################
  !#########################################################
  subroutine special_boundary_galbox(ilevel)
  ...
  !  use hydro_parameters !, ONLY: nvar,boundary_var,gamma,bx_bound,by_bound,bz_bound,turb,dens0,V0
    ...
    integer:: nx_loc, idim
    real(dp)::dx, dx_loc, scale, A, B, C
    real(dp), dimension(1:twotondim, 1:3)::xc
    real(dp), dimension(1:3)::skip_loc
  
    integer, dimension(1:nvector), save::ind_grid, ind_cell
    real(dp), dimension(1:nvector, 1:ndim), save::xx
    integer :: jgrid, ind_cell_vois
    real(dp)::dx_min, Emag
  ...
  return
    ...
    if (.not. use_special_boundary) return
      ...
      do ind = 1, twotondim
        ...
        do i = 1, ngrid
          ...
          if (uold(ind_cell(i), 1) .lt. 0d0) then
            write (*, *) "DENSITY < 0 BEFORE BOUNDARY_GALBOX, OH NO", ind_cell(i)
            call clean_stop
          end if
          if (uold(ind_cell(i), 5) .lt. 0d0) then
            write (*, *) "TOTAL CELL ENERGY < 0 BOUNDARY_GALBOX , OH NO", ind_cell(i)
            call clean_stop
          end if
          do j = 5, 8
            if (isnan(uold(ind_cell(i), j))) then
              write (*, *) "VARIABLE IS NAN BOUNDARY_GALBOX , OH NO", ind_cell(i), j, uold(ind_cell(i), 1)
              call clean_stop
            end if
          end do
          ...
          if (uold(ind_cell(i), 5) .lt. 0d0) then
            write (*, *) "TOTAL ENERGY < 0 AFTER BOUNDARY_GALBOX, OH NO", ind_cell(i)
            call clean_stop
          end if
          do j = 5, 8
            if (isnan(uold(ind_cell(i), j))) then
              write (*, *) "VARIABLE IS NAN AFTER BOUNDARY_GALBOX, OH NO", ind_cell(i), j
              call clean_stop
            end if
          end do
  ...
  end subroutine special_boundary_galbox
