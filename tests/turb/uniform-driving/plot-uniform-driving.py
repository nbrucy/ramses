...
ax[0].plot([0,1], [rho_init,rho_init], color='black', label='t=0.0')
ax[1].plot([0,1], [0,0], color='black')
...
for snap in range(2,7):
    ...
    # plot quantities
    ax[0].plot(x_sorted, rho_sorted, label='t={:.1f}'.format(0.2*(snap-1)), color=colorVals[snap])
    ax[1].plot(x_sorted, vx_sorted, color=colorVals[snap])
